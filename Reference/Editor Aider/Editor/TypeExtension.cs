...
namespace WanzyeeStudio.Extension{
	...
	public static class TypeExtension{
		...
		public static Type[] GetChildren(this Type type, bool deep = false){
			...
			var _all = AppDomain.CurrentDomain.GetAssemblies().SelectMany((assembly) => assembly.GetTypes());
		...
		public static bool IsCreatable(this Type type, bool exception = false){
			...
			try{
				...
				if(null == type) throw new ArgumentException("Argument cannot be null.");

				if(type.IsInterface) throw new ArgumentException("Can't create interface.");
				if(type.IsAbstract) throw new ArgumentException("Can't create abstract type.");

				if(type.IsGenericTypeDefinition) throw new ArgumentException("Can't create generic definition.");
				if(typeof(Delegate).IsAssignableFrom(type)) throw new ArgumentException("Can't create delegete.");
				...
				else return type.GetGenericArguments().All((arg) => arg.IsCreatable(exception));
